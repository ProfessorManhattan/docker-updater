...
> <br/>**A general-purpose Dockerfile project that includes Node.js, DockerSlim, and jq in a single container (only 220.469 MB compressed!)**<br/><br/>
...
docker-slim build --tag megabytelabs/updater:slim --http-probe=false --exec 'npx --help' --include-path '/usr/lib/libpcre2-8.so.0' --include-path '/usr/local/bin/docker-slim' --include-path '/usr/bin/jq' --include-path '/usr/bin/git' --include-path '/usr/bin/npx' --include-path '/bin/bash' --include-path '/usr/bin/npm' --include-path '/usr/bin/node' --include-path '/bin/sed' megabytelabs/updater:latest
